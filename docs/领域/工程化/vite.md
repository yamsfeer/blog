...
这个操作非常耗时，而且当文件内容发生改变，webpack 需要重新打包一次。即使采用了 HMR 模式，其热更新速度也会随着应用规模的增长而显著下降。

## vite 特性

Vite 以原生 ESM 方式提供源码，Vite 只需要在浏览器请求源码时进行转换并按需提供源码。根据情景动态导入代码，即只在当前屏幕上实际使用时才会被处理。
...
* 依赖通常存放在 `node_modules` 中
  * 浏览器无法直接访问 `node_modules`
  * 它们可以能有 ESM、CommonJS 等各种模块化格式
  * 单个模块可能导入了多个文件，比如 `lodash-es`
* 源码则是经常改变的
  * 并不是所有的源码都需要同时被加载，例如基于路由拆分的代码模块

为解决上述问题，vite 提出了两个解决方案：路径解析、依赖预构建。
...
来看一下 `/node_modules/.vite/deps/lodash-es.js?v=d45e265f` 文件的内容：

```javascript
import "/node_modules/.vite/deps/chunk-RSJERJUL.js?v=c2d0f0b4";
...
* 统一模块格式
  ...
  开发阶段，vite 将所有代码视为原生 ES 模块。因此 vite 先将作为 CommonJS 或 UMD 发布的依赖项转换为 ESM。
  ...
  ```javascript
  import __vite__cjsImport3_lodash from "/node_modules/.vite/deps/lodash.js?v=c2d0f0b4";
  const _ = __vite__cjsImport3_lodash.__esModule ? __vite__cjsImport3_lodash.default : __vite__cjsImport3_lodash;
  ```

预构建的第三方包会保存在本地 `node_modules/.vite/deps` 中，只有依赖发生变化，才会重新进行预构建。

第三方依赖请求会通过 `Cache-Control: max-age=31536000,immutable` 进行 http 强缓存，以提高在开发时的页面重载性能。一旦被缓存，这些请求将永远不会再到达开发服务器。
