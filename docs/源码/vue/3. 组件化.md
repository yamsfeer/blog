...
由于 `tag = App`，即 `App.vue` 导出的对象，因此这里会走到 `createComponent`。
...
上节的 App vnode 不包含内部节点的 vnode（没有 children），它只是个占位符，在 patch 后会被替换掉。
...
* `$mount` 会调用 mountComponent，里面有两个重要方法： `vm._update `和 `vm._render：`
  * `vm._render` 会执行 render 函数，即调用 `h(App)` 或 `$createElement(App)`
