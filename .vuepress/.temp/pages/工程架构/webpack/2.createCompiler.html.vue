<template><div><h1 id="createcompiler" tabindex="-1"><a class="header-anchor" href="#createcompiler"><span>createCompiler</span></a></h1>
<p>从用户角度，使用 webpack 打包的代码大概如下：</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> webpack <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">'webpack'</span><span class="token punctuation">)</span>
...
</code></pre></div><p>webpack 函数会调用 createCompiler 函数创建 compiler，它包含了大部分初始化工作。</p>
<p>createCompiler 函数主要做了以下几个工作：</p>
<ul>
<li>初始化参数</li>
<li>创建 Compiler 对象</li>
<li>注册用户插件和内置插件</li>
</ul>
<p>createCompiler 函数在源码中的调用关系如下图：</p>
<img src="https://raw.githubusercontent.com/yamsfeer/pic-bed/master/createCompiler.svg" style="zoom:90%;" />
<p>这里值得注意的有两点：创建 compiler 对象和注册内置插件。</p>
<ul>
<li>webpack 内置的插件非常多，需要注意的是 EntryOptionsPlugin，它处理的是入口相关的逻辑。</li>
</ul>
<h3 id="new-compiler" tabindex="-1"><a class="header-anchor" href="#new-compiler"><span>new Compiler</span></a></h3>
<p>compiler 对象上挂载了很多 hooks，它涵盖了构建过程的整个生命周期。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Compiler</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>插件通过监听 compiler 对象上的 hooks，可以在合适的时候介入编译打包过程。</p>
<h3 id="注册插件" tabindex="-1"><a class="header-anchor" href="#注册插件"><span>注册插件</span></a></h3>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">WebpackOptionsApply</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>在初始化阶段，需要注意的是 EntryOptionPlugin，是用于处理入口逻辑。</p>
<h3 id="处理入口文件" tabindex="-1"><a class="header-anchor" href="#处理入口文件"><span>处理入口文件</span></a></h3>
<p>在单文件入口的情况下，EntryOptionPlugin 会应用 EntryPlugin。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">EntryOptionPlugin</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>EntryPlugin 主要做了三件事：</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">EntryPlugin</span> <span class="token punctuation">{</span>
  <span class="token function">apply</span><span class="token punctuation">(</span><span class="token parameter">compiler</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>EntryPlugin 在 <code v-pre>compiler.hooks.make</code> 上挂载了监听函数，当调用 <code v-pre>compiler.run</code> 方法时会触发 make hook，进入 make 阶段。</p>
<h3 id="compiler-run" tabindex="-1"><a class="header-anchor" href="#compiler-run"><span>compiler.run</span></a></h3>
<p><code v-pre>compiler.run</code> 方法会调用 <code v-pre>compiler.compile</code> 方法，除了触发上面提到的 make hook 之外，compile 方法还会创建 compilation 对象处理编译工作。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Compiler</span> <span class="token punctuation">{</span>
...
<p>初始化阶段中，createCompiler 函数的主要工作：</p>
<ul>
<li>初始化参数，对 webpack.config.js 或 shell 命令传过来的参数进行 normalize 操作</li>
<li>创建 compiler 对象，初始化各种 hooks</li>
<li>注册内置插件和用户插件</li>
<li>创建 compilation 对象</li>
<li>触发 make hook，进入 make 阶段</li>
</ul>
</div></template>
