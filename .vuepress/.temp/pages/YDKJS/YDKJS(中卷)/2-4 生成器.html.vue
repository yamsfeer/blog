<template><div><h1 id="生成器" tabindex="-1"><a class="header-anchor" href="#生成器"><span>生成器</span></a></h1>
...
<h2 id="生成器的基本特点" tabindex="-1"><a class="header-anchor" href="#生成器的基本特点"><span>生成器的基本特点</span></a></h2>
<h3 id="输入和输出" tabindex="-1"><a class="header-anchor" href="#输入和输出"><span>输入和输出</span></a></h3>
<p>生成器函数是一个特殊的函数，它除了有函数的基本特性，比如接受参数和输出返回值，还可以<strong>中断函数的运行</strong>。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token operator">*</span><span class="token function">foo</span><span class="token punctuation">(</span><span class="token parameter">x<span class="token punctuation">,</span> y</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre></div><h3 id="双向消息传递" tabindex="-1"><a class="header-anchor" href="#双向消息传递"><span>双向消息传递</span></a></h3>
<p>生成器通过 <code v-pre>yield</code> 和 <code v-pre>next函数</code> 实现双向数据传递。</p>
...
</code></pre></div><p>我们并没有向第一个 next() 调用发送值，这是有意为之。<strong>只有暂停的 yield 才能接受这样一个通过 next 传递的值</strong>，而在生成器的起始处我们调用第一个 next() 时，还没有暂停的 yield 来接受这样一个值。规范和所有兼容浏览器都会默默<strong>丢弃</strong>传递给第一个 next() 的任何东西。因此，<strong>启动生成器时一定要用不带参数的 next()</strong>。</p>
<p>也许你会发现，next函数通常比yield多一个，事实上他们是一一对应的。<strong>next &lt;=&gt; yield + return</strong>。</p>
<h3 id="多个迭代器" tabindex="-1"><a class="header-anchor" href="#多个迭代器"><span>多个迭代器</span></a></h3>
...
</code></pre></div><h2 id="用生成器产生值" tabindex="-1"><a class="header-anchor" href="#用生成器产生值"><span>用生成器产生值</span></a></h2>
<p>“生成器”这个名称最初的使用场景就是作为一种产生值的方式。但是在介绍生成器如何产生值之前，我们先介绍下什么是迭代器。</p>
<h3 id="什么是迭代器" tabindex="-1"><a class="header-anchor" href="#什么是迭代器"><span>什么是迭代器</span></a></h3>
<p>假定你要产生一系列值，其中每个值都与前面一个有特定的关系。要实现这一点，需要一个有状态的生产者记住其生成的最后一个值。</p>
<p>我们可以用闭包实现：</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">var</span> genNum <span class="token operator">=</span> <span class="token keyword">void</span> <span class="token keyword">function</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre></div><p>另外，ES6 还新增了一个 for-of 循环，for-of 循环在每次迭代中自动调用迭代器的 next()，它不会向 next() 传入任何值，并且会在接收到 done:true 之后自动停止。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">for</span><span class="token punctuation">(</span><span class="token keyword">var</span> v <span class="token keyword">of</span> genNum<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><h3 id="iterable可迭代的" tabindex="-1"><a class="header-anchor" href="#iterable可迭代的"><span>iterable可迭代的</span></a></h3>
<p>实现了<code v-pre>Symbol.iterator</code>函数的对象是可迭代的（iterable），迭代器包含next函数，next 调用返回一个对象。这个对象有两个属性:done 是一个 boolean 值，标识迭代器的完成状态；value 中放置迭代值。</p>
...
<span class="token punctuation">}</span>
</code></pre></div><p>for-of 循环自动调用它的 Symbol.iterator 函数来构建一个迭代器。我们当然也可以手工调用这个函数，然后使用它返回的迭代器。</p>
...
it<span class="token punctuation">.</span><span class="token function">next</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">.</span>value<span class="token punctuation">;</span>
</code></pre></div><h3 id="生成器与迭代器" tabindex="-1"><a class="header-anchor" href="#生成器与迭代器"><span>生成器与迭代器</span></a></h3>
...
<h4 id="停止生成器-it-return" tabindex="-1"><a class="header-anchor" href="#停止生成器-it-return"><span>停止生成器 it.return()</span></a></h4>
...
<span class="token punctuation">}</span>
</code></pre></div><h4 id="抛出错误-it-throw" tabindex="-1"><a class="header-anchor" href="#抛出错误-it-throw"><span>抛出错误 it.throw()</span></a></h4>
<p>迭代器主动抛出错误，不常用。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token operator">*</span><span class="token function">foo</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre></div><h2 id="生成器重构回调例子" tabindex="-1"><a class="header-anchor" href="#生成器重构回调例子"><span>生成器重构回调例子</span></a></h2>
<p>先来看下回调的例子：</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">foo</span><span class="token punctuation">(</span><span class="token parameter">url<span class="token punctuation">,</span>cb</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre></div><p>生成器例子：</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">foo</span><span class="token punctuation">(</span><span class="token parameter">url</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre></div><h2 id="生成器-promise" tabindex="-1"><a class="header-anchor" href="#生成器-promise"><span>生成器 + promise</span></a></h2>
<p>同样地，先来回顾下之前promise的例子：</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token function">request</span><span class="token punctuation">(</span><span class="token string">'http://xx.com'</span><span class="token punctuation">)</span>
	...
	<span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre></div><p>生成器 + promise版本：</p>
...
<span class="token punctuation">)</span>
</code></pre></div><p>针对上面生成器 + promise版本的例子，可见我们需要每次小心地控制迭代器的运行，这不太方便且容易出错。</p>
<p>书中给了一个自动控制生成器执行的例子，这里仅供参考，因为在ES7中已经提供了标准化的<code v-pre>async / await</code>解决方法。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">run</span><span class="token punctuation">(</span><span class="token parameter">gen</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre></div><p>Async / await 版本：</p>
<p>如果你 await 了一个 Promise，async 函数就会自动获知要做什么，它会暂停这个函数(就像生成器一样)，直到 Promise 决议。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">async</span> <span class="token keyword">function</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token keyword">try</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h2 id="生成器委托" tabindex="-1"><a class="header-anchor" href="#生成器委托"><span>生成器委托</span></a></h2>
<h2 id="形实转换程序" tabindex="-1"><a class="header-anchor" href="#形实转换程序"><span>形实转换程序</span></a></h2>
<p><a href="http://www.ruanyifeng.com/blog/2015/05/thunk.html" target="_blank" rel="noopener noreferrer">thunk函数<ExternalLinkIcon/></a></p>
<h2 id="es6之前的生成器" tabindex="-1"><a class="header-anchor" href="#es6之前的生成器"><span>ES6之前的生成器</span></a></h2>
