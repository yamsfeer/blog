<template><div><h1 id="parse" tabindex="-1"><a class="header-anchor" href="#parse"><span>parse</span></a></h1>
<p>parser 需要完成词法分析和语法分析两个工作。</p>
<h2 id="词法分析" tabindex="-1"><a class="header-anchor" href="#词法分析"><span>词法分析</span></a></h2>
<p>词法分析通常是用状态机来完成的，解析 vue 模板的状态机如下图。</p>
<p>先定义各个状态。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> State <span class="token operator">=</span> <span class="token punctuation">{</span>
...
</code></pre></div><p>tokenize 过程就是从当前状态和字符，不断转移到下一个状态，并持续消耗 ( consume ) 当前字符的过程。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">tokenize</span><span class="token punctuation">(</span><span class="token parameter">str</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">]</span>
</code></pre></div><p>我们用正则表达式也可以完成相同的功能。实际上，正则表达式的本质就是状态机。</p>
<h2 id="构造-ast" tabindex="-1"><a class="header-anchor" href="#构造-ast"><span>构造 AST</span></a></h2>
<p>假设有如下模板。</p>
...
</code></pre></div><p>模板编译得到的 AST 是这样的。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> ast <span class="token operator">=</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">]</span>
<span class="token punctuation">}</span>
</code></pre></div><p>值得注意的是，AST 的根节点是 Root，它是逻辑上的根节点，模板中的节点都是 Root 的 children。</p>
<p>将 tokens 解析成 AST 的过程和 tokenize 有点类似，就是不断地 consume token，其中一些 token 会创建对应的 AST 节点，<strong>为了维护节点之间的父子关系</strong>，我们需要使用栈。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">analyse</span><span class="token punctuation">(</span><span class="token parameter">tokens</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><p>tokenize 和 analyse 函数分别执行词法分析和语法分析的过程。</p>
<div class="language-javascript" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">parse</span><span class="token punctuation">(</span><span class="token parameter">str</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
