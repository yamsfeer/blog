<template><div><h1 id="基于栈的虚拟机" tabindex="-1"><a class="header-anchor" href="#基于栈的虚拟机"><span>基于栈的虚拟机</span></a></h1>
<h3 id="如何运行二进制代码" tabindex="-1"><a class="header-anchor" href="#如何运行二进制代码"><span>如何运行二进制代码</span></a></h3>
<p>假设有如下代码。</p>
<div class="language-c++" data-ext="c++" data-title="c++"><pre v-pre class="language-c++"><code>int main() {
  int x = 1;
  int y = 2;
  int z = x + y;
  return z;
}
</code></pre></div><p>对应的汇编代码如下。</p>
<div class="language-assembly" data-ext="assembly" data-title="assembly"><pre v-pre class="language-assembly"><code>proc:	file format mach-o 64-bit x86-64
...
100003fb6: c3                          	retq
</code></pre></div><h3 id="栈如何管理函数调用" tabindex="-1"><a class="header-anchor" href="#栈如何管理函数调用"><span>栈如何管理函数调用</span></a></h3>
<p>执行函数时，函数的参数、内部变量都会依次压入到栈中。举个例子：</p>
<div class="language-c++" data-ext="c++" data-title="c++"><pre v-pre class="language-c++"><code>int main() {
  int x = 5;
  int y = 5;
  x = 100;
...
</code></pre></div><p>main 函数执行过程中，栈的变化如下图：</p>
<img src="https://raw.githubusercontent.com/yamsfeer/pic-bed/master/008i3skNgy1gweuzz9xybj31hc0u0di6.jpg" style="zoom: 35%;" />
<p>当一个函数调用另一个函数时：</p>
<div class="language-c++" data-ext="c++" data-title="c++"><pre v-pre class="language-c++"><code>int add(num1, num2) {
  int x = num1;
  int y = num2;
  int ret = x + y;
  return ret;
...
<p>在 add 函数中，我们得到了 add 函数的结果 ret，存放在栈顶：</p>
<img src="https://raw.githubusercontent.com/yamsfeer/pic-bed/master/008i3skNgy1gwev01s4o1j31rg0snjuk.jpg" style="zoom: 35%;" />
<p>add 函数执行完毕后，我们需要将栈的状态恢复到 main 函数上次执行时的状态，这个过程叫恢复现场。</p>
<p>为了恢复现场，在 main 函数调用 add 函数时，需要将当前栈顶指针 ( f92 ) 保存到 ebp 寄存器中，恢复现场时，只需将 ebp 中的值存放到 esp 即可。</p>
<p>恢复现场前：</p>
<img src="https://raw.githubusercontent.com/yamsfeer/pic-bed/master/008i3skNgy1gweuzztinlj31rg0ptn04.jpg" style="zoom: 25%;" />
<p>恢复现场后：</p>
<p>（注意 esp 的值由 f97 变成了 f92，图里没画出 ebp 寄存器，它的值是 f92）</p>
<img src="https://raw.githubusercontent.com/yamsfeer/pic-bed/master/008i3skNgy1gwev02pzmvj31rg0ptq65.jpg" style="zoom: 25%;" />
<p>这里还有一个问题，在执行 add 函数前，ebp 中是有值的，它的值是调用 main 函数前的栈顶地址（f91）。</p>
<p>为了恢复这个值，通常的方法是调用 add 函数前，将当前 main 函数的<strong>栈帧指针</strong>保存在栈中：（总的来说，就是寄存器不够，只能挪来挪去。）</p>
