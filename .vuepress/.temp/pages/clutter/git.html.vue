...
<p><img src="https://raw.githubusercontent.com/yamsfeer/pic-bed/master/git工作流.jpg" alt=""></p>
<div class="language-text" data-ext="text" data-title="text"><pre v-pre class="language-text"><code>- Workspace：工作区
- Index / Stage：暂存区
- Repository：仓库区（或本地仓库）
- Remote：远程仓库
</code></pre></div><div class="language-text" data-ext="text" data-title="text"><pre v-pre class="language-text"><code>start a working area (see also: git help tutorial)
...
</code></pre></div><h2 id="常用命令" tabindex="-1"><a class="header-anchor" href="#常用命令"><span>常用命令</span></a></h2>
<p>git basic:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> init <span class="token operator">&lt;</span>directory<span class="token operator">></span> <span class="token comment"># 在指定⽬录创建repo，不带参数将在当前⽬录创建</span>
...
</code></pre></div><p>git diff:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> <span class="token function">diff</span> <span class="token comment"># ⽐较⼯作区和暂存区的修改。</span>
...
</code></pre></div><p>rewriting git history:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> commit <span class="token parameter variable">-m</span> <span class="token operator">&lt;</span>message<span class="token operator">></span> <span class="token parameter variable">--amend</span> <span class="token comment"># 将当前staged修改合并到最近⼀次的commit中</span>
...
</code></pre></div><p>git branchs:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> branch <span class="token comment"># 显示本地所有分⽀</span>
...
</code></pre></div><p>remote repositories:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> remote <span class="token function">add</span> <span class="token operator">&lt;</span>name<span class="token operator">></span> <span class="token operator">&lt;</span>url<span class="token operator">></span> <span class="token comment"># 添加新的远程仓库，添加后可使⽤name作为远程连接的名称。</span>
...
</code></pre></div><p>git config:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> config <span class="token parameter variable">--global</span> user.name <span class="token operator">&lt;</span>name<span class="token operator">></span> <span class="token comment"># 配置⽤户名</span>
...
</code></pre></div><p>git log:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> log -<span class="token operator">&lt;</span>limit<span class="token operator">></span> <span class="token comment"># 限制log的显示数量。例如：”git log -5”仅显示最新5条commit。</span>
...
</code></pre></div><p>git reset:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> reset <span class="token comment"># 清除暂存区，⼯作区状态不变</span>
...
</code></pre></div><p>git rebase:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> rebase <span class="token parameter variable">-i</span> <span class="token operator">&lt;</span>startpoint<span class="token operator">></span> <span class="token operator">&lt;</span>endpoint<span class="token operator">></span> <span class="token comment"># 以交互模式rebase。</span>
</code></pre></div><p>git pull / git push:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> pull <span class="token operator">&lt;</span>remote<span class="token operator">></span> <span class="token comment"># 抓取所有远程分⽀并merge</span>
...
</code></pre></div><p>git tag:</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> tag <span class="token comment"># 查看所有标签</span>
...
</code></pre></div><h2 id="工作区和暂存区" tabindex="-1"><a class="header-anchor" href="#工作区和暂存区"><span>工作区和暂存区</span></a></h2>
<p>一个 git repo 是由 <code v-pre>.git</code> 文件夹管理的，里面有暂存区( stage 或 index )，各个分支( branch )，以及指向版本的 HEAD 指针，<code v-pre>.git</code> 之外的称为工作区。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ ll .git
...
</code></pre></div><p>工作区的所有修改，比如增加文件，删除文件，修改文件内容，都需要添加到暂存区，然后才能 commit。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> init
$ <span class="token function">touch</span> hello
$ <span class="token function">git</span> <span class="token function">add</span> hello
</code></pre></div><ol>
<li>
<p>如果修改了工作区文件的内容，需要丢弃工作区的修改，使用命令 <code v-pre>git checkout -- file</code></p>
</li>
<li>
<p>如果不但修改了工作区文件的内容，还添加到了暂存区，需要丢弃修改，先用命令 <code v-pre>git reset HEAD &lt;file&gt;</code> 回到第一种情况，然后 <code v-pre>git checkout -- file</code></p>
</li>
<li>
<p>已经提交了不合适的修改到版本库时，想要撤销本次提交，此时需要版本回退，使用命令 <code v-pre>git reset --hard HEAD^</code>，前提是没有推送到远程库。</p>
</li>
</ol>
<p><code v-pre>git checkout</code> 就是用版本库( 暂存区或已经 commit )版本替换工作区版本，无论工作区是修改还是删除，都可以替换。</p>
<h2 id="远程仓库-remote" tabindex="-1"><a class="header-anchor" href="#远程仓库-remote"><span>远程仓库( remote )</span></a></h2>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> remote <span class="token parameter variable">-v</span>                          <span class="token comment"># 查看已有远程仓库</span>
...
</code></pre></div><div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> remote <span class="token function">rm</span> hello
</code></pre></div><p>从存储库中删除远程 URL 只会取消本地和远程存储库的链接。 它不会删除远程存储库。</p>
<p>来做个小实验，先初始化一个 repo：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> init vscode
$ <span class="token builtin class-name">cd</span> vscode
</code></pre></div><p>此时还没有远程仓库，将 vscode 作为远程仓库地址，命名为 origin：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> remote <span class="token function">add</span> origin git@github.com:microsoft/vscode.git
...
</code></pre></div><p>如果有 commit 需要 push 到 github 的远程仓库的 master 分支：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> push origin master
</code></pre></div><p>将 origin 改名为 vscode：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> remote <span class="token function">rename</span> origin vscode
$ <span class="token function">git</span> remote <span class="token parameter variable">-v</span>
...
</code></pre></div><p>push 命令要相应改为</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> push vscode master
</code></pre></div><p>将 https 协议改为 ssh 地址：</p>
<div class="language-sshell" data-ext="sshell" data-title="sshell"><pre v-pre class="language-sshell"><code>$ git remote set-url vscode https://github.com/microsoft/vscode.git
$ git remote -v
vscode	https://github.com/microsoft/vscode.git (fetch)
vscode	https://github.com/microsoft/vscode.git (push)
</code></pre></div><p>删除远程仓库：</p>
<div class="language-text" data-ext="text" data-title="text"><pre v-pre class="language-text"><code>$ git remote rm vscode
</code></pre></div><p>从存储库中删除远程 URL 只会取消本地和远程存储库的链接。 它不会删除远程存储库。</p>
<h2 id="git-变基-rebase" tabindex="-1"><a class="header-anchor" href="#git-变基-rebase"><span>git 变基( rebase )</span></a></h2>
<p><code v-pre>git rebase</code> 可以对一段 commit 历史进行编辑，合理使用可以使提交历史变得简洁。</p>
<p>同时，和所有会改变提交历史的命令一样，在 commit 已经 push 到远端仓库的情况下不建议使用，否则会造成麻烦。</p>
<p>我们可以用 <code v-pre>git rebase</code> 合并多个 commit。</p>
...
</code></pre></div><p>其中 <code v-pre>--interactive</code> 表示弹出交互式界面来编辑具体合并操作，<code v-pre>[startpoint] [endpoint]</code> 则指定了 rebase 的编辑区间，<code v-pre>[endpoint]</code> 默认是 <code v-pre>HEAD</code>。( 注意这是一个<strong>左开右闭</strong>区间 )</p>
<p>假设当前有 4 个 commit，我们将新的 2 个 commit 合并为 1 个：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> log <span class="token parameter variable">--oneline</span>
...
61a55fb commit2
4af1183 commit1
</code></pre></div><div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> rebase <span class="token parameter variable">-i</span> 61a55fb <span class="token comment"># 61a55fb 是 commit2 的 commit id</span>
</code></pre></div><p>交互式编辑界面如下：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>pick f7233cf commit3
...
<span class="token comment">#</span>
</code></pre></div><p>可以看到 commit2 本身并不在其中，这验证了前面提到的左开右闭区间的特点。</p>
<p>上面注释中有各种操作的说明，比如</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>p, pick <span class="token operator">&lt;</span>commit<span class="token operator">></span> <span class="token operator">=</span> use commit
...
</code></pre></div><p>这里选几个操作说明。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>pick     <span class="token comment"># 使用 commit</span>
...
</code></pre></div><p>为了合并 commit，编辑效果如下：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>pick f7233cf commit3
squash 77b14bc commit4
</code></pre></div><p>保存后会提示编辑合并后的 commit message。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token comment"># This is a combination of 2 commits.</span>
...
</code></pre></div><p>输入 “combine commit” 后保存即可完成 rebase。rebase 后的提交历史：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> log <span class="token parameter variable">--oneline</span>
...
61a55fb commit2
4af1183 commit1
</code></pre></div><p>由于 git 历史记录被更改，因此通常的 <code v-pre>git push origin</code> <strong>不起</strong>作用。 此时需要“强制推送”：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token comment"># Don't override changes</span>
...
</code></pre></div><p>强制推送具有严重的影响，因为它会更改分支提交的历史顺序。 请谨慎使用，尤其是多人协作时。</p>
<h2 id="凭证系统-credentials-system" tabindex="-1"><a class="header-anchor" href="#凭证系统-credentials-system"><span>凭证系统( credentials system )</span></a></h2>
<p>在 clone 仓库时，如果使用的是 https 协议，则每次 push 或 pull 都需要输入账户密码。</p>
<p>git 默认是不缓存用户名密码的，需要一些配置。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> config <span class="token parameter variable">--global</span> credential.helper         <span class="token comment"># 查看当前缓存模式</span>
...
</code></pre></div><h3 id="git-内置凭证-cache-和-store" tabindex="-1"><a class="header-anchor" href="#git-内置凭证-cache-和-store"><span>git 内置凭证：cache 和 store</span></a></h3>
<p>git 提供了两种模式：<code v-pre>cache</code> 和 <code v-pre>store</code>。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> config <span class="token parameter variable">--global</span> credential.helper cache
</code></pre></div><p><code v-pre>cache</code> 模式下，git 会把登录信息保存在机器<strong>内存</strong>中一段时间，时间过后将被从内存中移除。一般是15分钟，可以添加参数自定义过期时间，比如 <code v-pre>--timeout 30000</code>。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> config <span class="token parameter variable">--global</span> credential.helper store
</code></pre></div><p><code v-pre>store</code> 模式下，git 会把登录信息保留在<strong>硬盘</strong>中，且<strong>没有失效时间</strong>，除非修改密码或主动取消该模式。</p>
<p>该模式存在安全问题，<strong>密码信息是用明文保存的</strong>。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">cat</span> ~/.git-credentials 
https://username:ghp_Wkl<span class="token punctuation">..</span><span class="token punctuation">..</span>.xxxx@github.com
</code></pre></div><h3 id="其他凭证系统" tabindex="-1"><a class="header-anchor" href="#其他凭证系统"><span>其他凭证系统</span></a></h3>
<h4 id="oxskeychain-模式-mac" tabindex="-1"><a class="header-anchor" href="#oxskeychain-模式-mac"><span>oxskeychain 模式( mac )</span></a></h4>
<p>此模式下会将登录凭证加密处理并存储在 mac 的 osxkeychain 中。可以用 Keychain Access 查看。</p>
<p><img src="https://raw.githubusercontent.com/yamsfeer/pic-bed/master/oxskeychain.jpg" alt=""></p>
<p>也可以通过命令删除凭据：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> credential-osxkeychain erase
...
</code></pre></div><h4 id="wincred-模式-windows" tabindex="-1"><a class="header-anchor" href="#wincred-模式-windows"><span>wincred 模式( windows )</span></a></h4>
<p>该模式会将凭证信息加密处理放入到 windows 凭据管理器中。</p>
<h4 id="manager-模式-windows" tabindex="-1"><a class="header-anchor" href="#manager-模式-windows"><span>manager 模式( windows )</span></a></h4>
<p><code v-pre>manager</code> 模式需要安装名为 <code v-pre>Git Credential Manager for Windows</code> 的 <a href="https://github.com/Microsoft/Git-Credential-Manager-for-Windows." target="_blank" rel="noopener noreferrer">Git凭证助手程序<ExternalLinkIcon/></a>。该模式与 <code v-pre>osxkeychain</code> 模式类似，都会对凭证信息加密处理。</p>
<h4 id="gcm-凭证助手-推荐" tabindex="-1"><a class="header-anchor" href="#gcm-凭证助手-推荐"><span>GCM 凭证助手( 推荐 )</span></a></h4>
<p>GCM 是微软开发的凭证助手 <code v-pre>Git Credential Manager Core</code> 的简称。</p>
<p>安装命令：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ brew <span class="token function">install</span> <span class="token parameter variable">--cask</span> git-credential-manager-core
</code></pre></div><p>对于 mac，不需要运行 <code v-pre>git config</code>，GCM 会自动配置 git。</p>
<p>首次验证成功后，凭据会存储在 mac 的 osxkeychain 中，每次克隆 HTTPS URL 时都会使用。 Git 不会要求在命令行中再次键入凭据，除非更改凭据。</p>
<h3 id="store模式-修改密码后如何处理" tabindex="-1"><a class="header-anchor" href="#store模式-修改密码后如何处理"><span>store模式，修改密码后如何处理？</span></a></h3>
<p>store 模式下，凭证被存储到本地硬盘中。修改密码后，本地还存储着旧的凭证，导致仓库权限认证失败。</p>
<p>输入命令移除凭证：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> config <span class="token parameter variable">--global</span> <span class="token parameter variable">--unset</span> credential.helper
</code></pre></div><p>需要注意的是，这条命令不会把已缓存的凭证删除，因此需要手动删除旧凭证。</p>
<p>windows 保存在 <code v-pre>C:\Users\xxx\.git-credentials</code>，linux 为 <code v-pre>~/.git-credentials</code>。</p>
<p>删除后，重新设置 store 模式，然后执行 pull /push 操作会提示输入凭证，输入新密码就可以了。</p>
<h3 id="小结" tabindex="-1"><a class="header-anchor" href="#小结"><span>小结</span></a></h3>
<table>
<thead>
<tr>
<th style="text-align:center">登录凭证存储</th>
...
<tbody>
<tr>
<td style="text-align:center">内存中，短期失效</td>
<td style="text-align:center">cache</td>
<td style="text-align:center">cache</td>
<td style="text-align:center">cache</td>
</tr>
<tr>
<td style="text-align:center">硬盘，明文存储，永久</td>
<td style="text-align:center">store</td>
<td style="text-align:center">store</td>
<td style="text-align:center">store</td>
</tr>
<tr>
<td style="text-align:center">硬盘，加密存储，永久（推荐）</td>
<td style="text-align:center">GCM</td>
<td style="text-align:center">osxkeychain、GCM</td>
<td style="text-align:center">wincred、manager、GCM</td>
</tr>
</tbody>
</table>
<h2 id="几个技巧" tabindex="-1"><a class="header-anchor" href="#几个技巧"><span>几个技巧</span></a></h2>
<h3 id="修改最后一次-commit" tabindex="-1"><a class="header-anchor" href="#修改最后一次-commit"><span>修改最后一次 commit</span></a></h3>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> commit <span class="token parameter variable">--amend</span> <span class="token parameter variable">-m</span> <span class="token string">"new message"</span>
</code></pre></div><p>以上命令可以将本次提交覆盖上一次提交。</p>
<p>使用场景有两个，比如上一次提交有个小错误，修复后没必要提交一个新的 commit，又或者想修改上一个 commit message，都可以用 <code v-pre>--amend</code> 修正。</p>
<p>需要注意的是，既然是覆盖上一个 commit，如果覆盖前的历史已经 push 到远端，在多人协作开发的情况下，可能会造成严重的冲突，此时不宜使用。</p>
<h3 id="修复-bug" tabindex="-1"><a class="header-anchor" href="#修复-bug"><span>修复 bug</span></a></h3>
<p>如果开发过程中，暂存区和工作区都有未 commit 的修改，此时需要切换到另一个分支修复 bug。为保存这些还未完成的工作，有两个做法：</p>
<ul>
<li>临时 commit 一次，以后再用 <code v-pre>git commit --amend</code> 修正</li>
<li>用 <code v-pre>git stash</code> 将工作现场保存下来</li>
</ul>
<p>需要注意的是，<code v-pre>git stash</code> 不会保存工作区的修改，因此需要先执行 <code v-pre>git add .</code> 将所有内容保存到 stage。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> <span class="token function">add</span> <span class="token builtin class-name">.</span>
...
</code></pre></div><p>可以看到生成了一条 stash 记录，<code v-pre>git stash</code> 可以执行多次，以栈的方式存储起来。</p>
<p>此时可以切换到其他分支进行工作，完成之后再恢复状态。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> stash apply <span class="token comment"># 恢复但不删除</span>
...
</code></pre></div><h4 id="cherry-pick" tabindex="-1"><a class="header-anchor" href="#cherry-pick"><span>cherry-pick</span></a></h4>
<p>假设你在 master 分支修复了 bug，并回到当前 dev 分支，很明显 dev 分支还有没修复这个 bug。</p>
<p>为此，可以执行：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> cherry-pick <span class="token operator">&lt;</span>commit-id<span class="token operator">></span>
</code></pre></div><p>将 master 分支上修复 bug 的 commit 在当前分支重做一次，并进行一次新的提交。注意这两个commit只是改动相同，但它们是两个不同的commit。</p>
<h3 id="别名" tabindex="-1"><a class="header-anchor" href="#别名"><span>别名</span></a></h3>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> config <span class="token parameter variable">--global</span> alias.<span class="token operator">&lt;</span>alias-name<span class="token operator">></span> <span class="token operator">&lt;</span>git-command<span class="token operator">></span>
</code></pre></div><p>以上命令可以为一个命令起一个简短的别名，比如：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> config <span class="token parameter variable">--global</span> alias.glog log <span class="token parameter variable">--graph</span> <span class="token parameter variable">--oneline</span>
</code></pre></div><p>此时以下两个命令是等效的。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> glog
...
</code></pre></div><p><code v-pre>--global</code> 表示对当前用户起作用，不加则只针对当前仓库。</p>
<p>别名配置可以在 <code v-pre>~/.gitconfig</code> 中查看，配置别名也可以直接编辑这个文件。</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">cat</span> ~/.gitconfig
...
</code></pre></div><h3 id="快速-clone-大仓库" tabindex="-1"><a class="header-anchor" href="#快速-clone-大仓库"><span>快速 clone 大仓库</span></a></h3>
<p>一些项目经过长时间的持续开发，积累了很长的历史记录，clone 这样一个 repo 需要消耗相当长时间。</p>
<p>如果我们不关心具体的提交历史，只关心最新提交的代码，以 chromium 为例：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> clone https://github.com/chromium/chromium.git <span class="token parameter variable">--depth</span><span class="token operator">=</span><span class="token number">1</span>
</code></pre></div><p>这样 clone 完成后，仓库中只包含了最后一次 commit 的信息，下载速度和仓库体积都大幅减小。</p>
<p>如果需要下载某个 tag，先在本地创建一个 repo：</p>
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>$ <span class="token function">git</span> init chromium
...
<div class="language-bash" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 下载特定 tag 的最后一次 commit</span>
...
<p><a href="https://docs.github.com/cn/get-started/getting-started-with-git" target="_blank" rel="noopener noreferrer">GitHub Docs<ExternalLinkIcon/></a></p>
<p><a href="https://www.panyanbin.com/article/7a09727c.html" target="_blank" rel="noopener noreferrer">git自动保存账号密码<ExternalLinkIcon/></a></p>
<p><a href="https://www.liaoxuefeng.com/wiki/896043488029600" target="_blank" rel="noopener noreferrer">git 教程<ExternalLinkIcon/></a></p>
<p><a href="https://www.ruanyifeng.com/blog/2015/12/git-cheat-sheet.html" target="_blank" rel="noopener noreferrer">常用 Git 命令清单<ExternalLinkIcon/></a></p>
</div></template>
